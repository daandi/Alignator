...
class WordAlignator
    ...
    def alignate
        @ocr_line.each_with_index do |ocr_word, pos|
            ...
            # Problem mit nicht wohlgeformten Strings (utf-8-Problem bei Levenshtein)
            begin
                distance = Text::Levenshtein.distance( ocr_word, ocrx_word.text)
            rescue
                puts "Problem mit #{ocrx_word.text} bei Ermittlung des Distanzmaßes, Überspringe dieses Wort"
                next
            end
    ...
    def distance_if_merge_at(pos)
        ...
        if weak_ocr_word and weak_ocr_word.text.length >= ocr_word.length then
            weak_ocr_sub_sequence = weak_ocr_word.text.scan(/./u)[0..ocr_word.length() - 1].join("")
    ...
    def join_splitted_word(split_start, count)
        ...
        # Testen ob über mehrere Zeilen geht
        unless split[0].x1 > split[-1].x2 then
        	OCRXWord.new( split[0].x1, y1_min, split[-1].x2, y2_max, fixed_word)
        else
        	OCRXWord.new( split[0].x1, y1_min, split[0].x2, y2_max, fixed_word)
        end
    end
